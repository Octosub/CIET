class Gpt
  ...
  def self.classify_ingredient(ingredient)
    prompt_vegan = "Is #{ingredient.name} vegan? Answer only 'true' or 'false'"
    ...
    if vegan_response.downcase.include?("true")
      ...
      prompt_vegetarian = "Is #{ingredient.name} vegetarian? Answer only 'true' or 'false'"
      ...
      if vegetarian_response.downcase.include?("true")
        ...
        prompt_pescetarian = "Is #{ingredient.name} pescetarian? Answer only 'true' or 'false'"
      ...
      prompt_dairy_free = "Is #{ingredient.name} dairy-free? Answer only 'true' or 'false'"
    ...
    prompt_peanut_free = "Is #{ingredient.name} peanut-free? Answer only 'true' or 'false'"
  ...
  def self.describe_ingredient(ingredient)
    prompt = <<~PROMPT
    "Describe #{ingredient.name} in 1 sentence, how it is produced in 1 sentence, and whether it's #{All_PREFERENCES} in 1 short sentence."
    PROMPT
